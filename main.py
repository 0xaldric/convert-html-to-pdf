...
os.makedirs(PDF_OUTPUT_DIR, exist_ok=True)  # Create directory if it doesn't exist

# PDFKit configuration (Path to wkhtmltopdf binary if needed)
PDFKIT_CONFIG = pdfkit.configuration(wkhtmltopdf='/usr/local/bin/wkhtmltopdf')  # Update path as necessary
...
async def generate_pdf(input: HTMLInput):
    ...
    try:
        ...
        # Convert HTML to PDF
        data = pdfkit.from_string(input.html, output_path, configuration=PDFKIT_CONFIG)
